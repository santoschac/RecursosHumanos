...
if($_SESSION['IdSucursal']){
    ...
    $sql1= $pdo->prepare("SELECT p.IdPersonal, p.Nombre, p.ApellidoPaterno, p.ApellidoMaterno, p.Departamento, pu.NombrePuesto, s.IdSucursal, s.NombreSucursal, e.NombreEmpresa, por.IdComisionPorcentaje, por.Porcentaje
    from personal p
    inner join puestos pu on p.IdPuesto= pu.IdPuesto
    inner join sucursal s on p.IdSucursal = s.IdSucursal
    inner join empresa e on s.IdEmpresa = e.IdEmpresa
    inner join comisionporcentaje por on p.IdPersonal = por.IdPersonal where s.IdSucursal = $IdSucursal");
$sql1->execute();
